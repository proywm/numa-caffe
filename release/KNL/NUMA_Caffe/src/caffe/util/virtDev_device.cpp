...
bool VirtDevManager::virtDevAssignOMPThreads()
{
	...
	cpu_set_t my_set;        /* Define your cpu_set bit mask. */
        ...
        CPU_SET((sched_getcpu()%2)+omp_get_thread_num()*2, &my_set);
...
{
//        int orgCPU = (COARSE_THREADS*(deviceID%VIRTDEVSCOUNT))+omp_get_thread_num();
        cpu_set_t my_set;        /* Define your cpu_set bit mask. */
        CPU_ZERO(&my_set);       /* Initialize it all to 0, i.e. no CPUs selected. */
        int a = 0;
                if(omp_get_thread_num()>= 0 && omp_get_thread_num() < COARSE_THREADS)
                {
                        CPU_SET((COARSE_THREADS*(deviceID%VIRTDEVSCOUNT))+(64*0)+(omp_get_thread_num()%COARSE_THREADS), &my_set);     /* set the bit that represents core 7. */
                        a = (COARSE_THREADS*(deviceID%VIRTDEVSCOUNT))+(64*0)+(omp_get_thread_num()%COARSE_THREADS);
                }
                else if(omp_get_thread_num()>= COARSE_THREADS && omp_get_thread_num() < 2*COARSE_THREADS)
                {
                        CPU_SET((COARSE_THREADS*(deviceID%VIRTDEVSCOUNT))+(64*1)+(omp_get_thread_num()%COARSE_THREADS), &my_set);
                        a = (COARSE_THREADS*(deviceID%VIRTDEVSCOUNT))+(64*1)+(omp_get_thread_num()%COARSE_THREADS);
                }
                else if(omp_get_thread_num()>= 2*COARSE_THREADS && omp_get_thread_num() < 3*COARSE_THREADS)
                {
                        CPU_SET((COARSE_THREADS*(deviceID%VIRTDEVSCOUNT))+(64*2)+(omp_get_thread_num()%COARSE_THREADS), &my_set);
                        a = (COARSE_THREADS*(deviceID%VIRTDEVSCOUNT))+(64*2)+(omp_get_thread_num()%COARSE_THREADS);
                }
                else if(omp_get_thread_num()>= 3*COARSE_THREADS && omp_get_thread_num() < 4*COARSE_THREADS)
                {
                        CPU_SET((COARSE_THREADS*(deviceID%VIRTDEVSCOUNT))+(64*3)+(omp_get_thread_num()%COARSE_THREADS), &my_set);
                        a = (COARSE_THREADS*(deviceID%VIRTDEVSCOUNT))+(64*3)+(omp_get_thread_num()%COARSE_THREADS);
                }
		else
...
/*
        ...
        {
                CPU_SET(orgCPU+64, &my_set);
		CPU_SET(orgCPU+128, &my_set);
		CPU_SET(orgCPU+192, &my_set);
        }
*/
	//if(a>15 && a<64)
...
{
        int orgCPU = COARSE_THREADS*(deviceID%VIRTDEVSCOUNT);
        cpu_set_t my_set;        /* Define your cpu_set bit mask. */
        CPU_ZERO(&my_set);       /* Initialize it all to 0, i.e. no CPUs selected. */

	for(int c=0 ;c< COARSE_THREADS;c++)
        	CPU_SET(orgCPU+c, &my_set);     /* set the bit that represents core 7. */
#if 0
        CPU_SET(orgCPU+1, &my_set);
        CPU_SET(orgCPU+2, &my_set);
        CPU_SET(orgCPU+3, &my_set);
        CPU_SET(orgCPU+4, &my_set);     /* set the bit that represents core 7. */
        CPU_SET(orgCPU+5, &my_set);
        CPU_SET(orgCPU+6, &my_set);
        CPU_SET(orgCPU+7, &my_set);
#endif
...
#if 0
        CPU_SET(orgCPU+2, &my_set);
	CPU_SET(orgCPU+4, &my_set);
	CPU_SET(orgCPU+6, &my_set);
        CPU_SET(orgCPU + 8, &my_set);     /* set the bit that represents core 7. */
        CPU_SET(orgCPU+10, &my_set);
        CPU_SET(orgCPU+12, &my_set);
        CPU_SET(orgCPU+14, &my_set);
#endif
