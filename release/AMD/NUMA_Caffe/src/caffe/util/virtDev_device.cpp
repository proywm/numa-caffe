...
//        int orgCPU = (COARSE_THREADS*(deviceID%VIRTDEVSCOUNT))+omp_get_thread_num();
        ...
        int a = 0;
                ...
                {
                        CPU_SET((COARSE_THREADS*(deviceID%VIRTDEVSCOUNT))+(omp_get_thread_num()%COARSE_THREADS), &my_set);     /* set the bit that represents core 7. */
                        a = (COARSE_THREADS*(deviceID%VIRTDEVSCOUNT))+(omp_get_thread_num()%COARSE_THREADS);
                }
		else
