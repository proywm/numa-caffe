...
int VirtDevManager::placeCOARSEOMPWithinSocket(int deviceID)//solverID
{
        ...
        int a = 0;
                ...
                {
                        CPU_SET((deviceID%VIRTDEVSCOUNT)*COARSE_THREADS+omp_get_thread_num(), &my_set);     /* set the bit that represents core 7. */
                        a = (deviceID%VIRTDEVSCOUNT)*COARSE_THREADS+omp_get_thread_num();
                }
                else if(omp_get_thread_num()>= COARSE_THREADS && omp_get_thread_num() < 2*COARSE_THREADS)
                {
			CPU_SET((deviceID%VIRTDEVSCOUNT)*COARSE_THREADS+omp_get_thread_num()+32, &my_set);     /* set the bit that represents core 7. */
                        a = (deviceID%VIRTDEVSCOUNT)*COARSE_THREADS+omp_get_thread_num()+32;
                }
		else
...
//        mkl_set_num_threads_local(1);
		...
		cpu_set_t my_set;
		CPU_ZERO(&my_set);
		CPU_SET((deviceID%VIRTDEVSCOUNT)*COARSE_THREADS+omp_get_thread_num(), &my_set);
...
bool VirtDevManager::virtDevSetDevice(int deviceID){
        ...
        //int orgCPU = deviceID;
	int orgCPU = (deviceID%VIRTDEVSCOUNT)*COARSE_THREADS;
        cpu_set_t my_set;        /* Define your cpu_set bit mask. */
        CPU_ZERO(&my_set);       /* Initialize it all to 0, i.e. no CPUs selected. */
	int c = 0;
       // for(int c=0 ;c< COARSE_THREADS;c++)
